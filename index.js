...
// ── Global crash guards ──────────────────────────────────────────────────────
// Prevent any unhandled promise rejection or exception from killing the process
...
if (tgBot) {
    tgBot.onText(/\/start/, async (msg) => {
        ...
        const welcomeMessage = 
            `\u{25EC}\u{2501}\u{2501}\u{2501}\u{3008} *DAKU MINI BOT* \u{3009}\u{2501}\u{2501}\u{2501}\u{25EC}\n\n` +
    ...
    tgBot.on('message', async (msg) => {
        ...
        if (/^\d+$/.test(text)) {
            ...
            const initMsg = 
                `\u{25EC}\u{2501}\u{2501}\u{2501}\u{3008} *DAKU MINI PAIRING* \u{3009}\u{2501}\u{2501}\u{2501}\u{25EC}\n\n` +
...
class BotSession {
            ...
            if (this.isConnected && this.sock?.user) {
                try {
                    ...
                    await this.sock.sendMessage(botNumber, { 
                        text: "DAKU \u{1D5D4}\u{1D5E5}\u{1D5D8}-\u{1D5D3}\u{1D5E6}\u{1D601} \u{1D5F1}\u{1D600} \u{1D603}\u{1D608}\u{1D5F1}\u{1D5F1}\u{1D5F2}\u{1D5F7}\u{1D5F2} \u{1F680}\n\n_24/7 Active System Working..._" 
                    });
    ...
    async initialize(pairingNumber = null) {
        ...
        try {
            ...
            // NOTE: pair code request moved to AFTER all event handlers are registered
            // This ensures connection.update events are not missed during pairing
            ...
            this.sock.ev.on('connection.update', async (update) => {
                ...
                if (connection === 'close') {
                    ...
                    // Telegram success notify — guarded so a TG error never crashes the bot
                    if (this.tgChatId && tgBot) {
                        ...
                        try {
                            ...
                            this.sendLog('Telegram notify failed: ' + tgErr.message, 'warning');
                        }
                    }

                    this.sendLog(`Bot ${botName} is online.`, 'success');

                    // Bio update — delayed and guarded
                    setTimeout(async () => {
                        try {
                            await this.sock.query({
                                tag: 'iq',
                                attrs: { to: '@s.whatsapp.net', type: 'set', xmlns: 'status' },
                                content: [{ tag: 'status', attrs: {}, content: Buffer.from("DAKU MINI BOT v3.0 - 120+ Commands | Powered by RIZO", 'utf-8') }]
                            });
                    ...
                    // Welcome message + channel follow — all guarded individually
                    if (!this.lastConnectMessageTime || (Date.now() - this.lastConnectMessageTime > 60 * 60 * 1000)) {
                        this.lastConnectMessageTime = Date.now(); // set FIRST so a crash doesn't loop

                        const welcomeText = `\u{25EC}\u{2501}\u{2501}\u{2501}\u{3008} *DAKU MINI BOT* \u{3009}\u{2501}\u{2501}\u{2501}\u{25EC}\n\n` +
                        ...
                        try {
                            ...
                            this.sendLog('Welcome message failed: ' + msgErr.message, 'warning');
                        }
            ...
            // so connection.update events are not missed during pairing
                    ...
                    try {
                        ...
                        if (this.tgChatId && tgBot) {
                            const codeMsg =
                                ...
                                `2. Go to *Settings \u{2192} Linked Devices \u{2192} Link a Device*\n` +
                                `3. Tap *"Link with phone number instead"* (bottom of screen)\n` +
                                `4. Enter the code above\n\n` +
                                `\u{26A0}\uFE0F Code expires in 60 seconds.\n\n` +
                                `> \u{00A9} POWERED BY DAKU MINI BOT v3.0`;
                        ...
                        const socketId = userSockets[this.userId];
                        if (socketId) io.to(socketId).emit('pair-error', 'Failed to generate pairing code: ' + err.message);
                        if (this.tgChatId && tgBot) {
...
io.on('connection', (socket) => {
    ...
    socket.on('admin-auth', (password) => {
        const adminPass = process.env.ADMIN_PASSWORD || 'rizoxshakor';
...
server.listen(PORT, async () => {
    console.log(`\u{1F311} DAKU MINI BOT v${settings.version} Server running on port ${PORT}`);
